...
function normalizeProduct(input, branchId) {
  ...
  return {
    ...
    inverterType: input.inverterType ?? input.technology ?? "",
...
function legacyRowToNormalizeInput(legacy) {
  return {
    ...
    inverterType: legacy.inverterType ?? legacy.technology,
...
export async function createProduct(data, { branchId = null } = {}) {
  ...
  row = {
    ...
    modelType: data.modelType ?? row.modelType,
    inverterType: data.inverterType ?? row.inverterType,
...
export async function updateProduct(id, data, { branchId = null, isSuperAdmin = false } = {}) {
  ...
  const merged = {
    ...
    modelType: data.modelType ?? legacy.modelType,
    inverterType: data.inverterType ?? legacy.inverterType ?? legacy.technology,
