...
import { resolveProductImagePath } from "../constants/brandImages.js";
...
export async function renderInvoiceHtml(invoice) {
  ...
  if (invoice.inverter || invoice.battery) {
    const batPath = invoice.battery
      ? resolveProductImagePath({
          brand: invoice.battery.brand,
          model: invoice.battery.model,
          product: "battery",
        }).path
      : "";
    const invPath = invoice.inverter
      ? resolveProductImagePath({
          brand: invoice.inverter.brand,
          model: invoice.inverter.model,
          product: "inverter",
        }).path
      : "";
