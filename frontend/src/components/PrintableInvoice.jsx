...
export function PrintableInvoiceDocument({ invoice, printId = "invoice-print-area" }) {
  ...
  const invThumb =
    inv?.model != null && String(inv.model).trim()
      ? publicAssetHref(
      ...
      : "";
  const batThumb =
    bat?.model != null && String(bat.model).trim()
      ? publicAssetHref(
          resolveProductImagePath({
            brand: bat.brand,
            model: bat.model,
    ...
    <div id={printId} className="invoice-a4-sheet">
      ...
      <div className="invoice-a4-header">
        <div className="invoice-a4-logo-wrap">
          <img src={publicAssetHref(LOGO_SRC)} alt="BatteryMela" className="invoice-a4-logo" />
        </div>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 10, alignItems: "center" }}>
              {invThumb ? (
                <img
                  src={invThumb}
                  alt=""
                  style={{ width: 52, height: 52, objectFit: "contain", border: "1px solid #e5e7eb", borderRadius: 6, background: "#f8fafc" }}
                />
              ) : null}
              {batThumb ? (
                <img
